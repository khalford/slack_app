...
class PostDataToInflux:
    ...
    def __init__(self):
        ...
        self.client = influxdb_client.InfluxDBClient(url=self.host, org=self.org, token=self.token)
    ...
    def write_to_influx(self, prs: Dict[str, List]):
        ...
        for repo in prs.keys():
            point = influxdb_client.Point("repository_pr_count").tag("repository", repo).field("no-prs",len(prs[repo])).time(time_ns())
